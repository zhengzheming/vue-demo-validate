  ...
  computed: {
      ...
      return Object.keys(_$vParams).map(key => {
        ...
        if (typeof _$vKeys !== "undefined" && _$vKeys.hasOwnProperty(key)) {
          _$vKeys[key].params.forEach(param => {
            ...
            params[param.ext] =
              this.validatorParams[param.other] || _$vParams[key][param.vue];
          });
          return getValidationObject.call(
            this,
            _$vKeys[key].validationKey,
            key,
            params
          );
        } else if (_$vParams[key] && Object.keys(_$vParams[key]).length) {
          // If the current validator key has params at all
          // We haven't defined a validation in our validationKeys setting so we try to map the Vuelidate params.
          Object.keys(_$vParams[key])
            .filter(k => k !== "type")
            .forEach(k => {
    ...
    mergedMessages() {
      return Object.assign(
        {},
        this.$vuelidateErrorExtractor.messages,
        this.messages
      );
    },
    firstError() {
    ...
    firstErrorMessage() {
      return this.hasErrors
        ? this.getErrorMessage(
            this.firstError.validationKey,
            this.firstError.params
          )
        : "";
    },
    hasErrors() {
  ...
  methods: {
    getErrorMessage(key, properties) {
      return this.$vuelidateErrorExtractor.i18n
        ? this.getI18nMessage(key, properties)
        : this.getPlainMessage(key, properties);
    },
    getI18nMessage(key, properties) {
      return this.$t(
        this.$vuelidateErrorExtractor.i18n + "." + key,
        properties
      );
    },
