...
export function get(string, object, fallback = "") {
  if (typeof string !== "string") {
    idDev &&
      console.warn(
        `Expected a string in the first argument, got ${typeof string}`
      );
    return fallback;
  }
  ...
  if (typeof object !== "object") {
    idDev &&
      console.warn(
        `Expected an Object/Array in the second argument, got ${typeof object}`
      );
    return fallback;
  }
  ...
  try {
    return string
      .split(".")
      .reduce((obj, current) => obj[current] || "", object);
  } catch (err) {
...
export function getValidationObject(
  ...
  return {
    ...
    params: Object.assign(
      {},
      { attribute: this.attribute || this.label, label: this.label },
      params,
